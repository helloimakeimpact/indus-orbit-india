-- Keep role-mention side effects replay-safe when a client retries the same message
-- after losing the original response. The durable mention row is the side-effect gate.
...
comment on function public.send_my_conversation_message_with_audience(
  ...
  'Sends one replay-safe message with bounded person and manager-only role mentions; role notification side effects occur only when the durable role mention is first inserted.';
