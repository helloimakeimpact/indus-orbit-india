...
| I/O Port                      | Product shell, control plane, local OpenCode proof, registry-driven gateway, latest-evidence routing and product split are Released to demo; live inventory is empty         | Reviewed provider provisioning, conformance, idempotency/health/budget controls, ledger, durable terminal, 20-provider inventory and full operator UI remain |
| Supabase platform             | Active demo project; ten current migrations, 20/20 hosted checks and the upgraded-local 376/376 database suite pass; provider inventory/routing/traffic are zero             | 26 historical aliases, fresh 61-migration CI, managed Realtime, authenticated function audits and performance-policy backlog remain                          |
...
- 45 authenticated `SECURITY DEFINER` warnings require contract-by-contract review; current Security Advisor output also contains 47 GraphQL-exposure and two always-true-policy warnings;
...
- current warning-level Performance Advisor results include 134 RLS init-plan and 77 multiple-permissive-policy findings; 118 index notices are informational and no error-level advisor issue exists;
...
- provider/model/endpoint/capacity/runtime-control inventory is currently empty, so provider keys alone cannot activate routing; provisioning and conformance remain required;
...
- **Released to demo:** ten current migrations are recorded and the read-only release contract passes 20/20; trusted product events are RPC-only, provider inventory/ready routes and provider traffic evidence are zero;
- **Security improvement:** anonymous execution of the identified privileged surface is closed and the legacy arbitrary-email function is a deployed 410 tombstone; 45 authenticated definer, 47 GraphQL exposure and two always-true-policy warnings require classification/remediation;
- **Verified on the upgraded local chain:** all ten pgTAP files pass 376/376; `.github/workflows/database.yml` still needs a fresh 61-migration empty replay and retained remote-run evidence;
