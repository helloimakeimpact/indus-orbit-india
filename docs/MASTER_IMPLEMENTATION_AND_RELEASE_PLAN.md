...
| Conversations                 | Direct messages plus Released Chapter/Mission Space schema, deterministic Rooms and first branded Space UI exist                                                             | Add hosted browser personas, shared store, cursor pagination, Threads UI, private Broadcast, explicit blocking and reliability/load tests                   |
...
| I/O Port                      | Product shell, control plane, local OpenCode proof, gateway and five-provider/three-capacity inventory are Released; traffic evidence is zero                                | Secret-reference validation, conformance, idempotency/health/budget controls, ledger, durable terminal, 20-provider inventory and operator UI remain        |
| Supabase platform             | Active demo project; all 62 migrations, clean 433/433 local suite and hosted Space schema/RLS/RPC/Realtime contract pass                                                     | 26 historical aliases, snapshot-upgrade/remote CI, authenticated function audits and inherited performance-policy backlog remain                            |
...
- **Verified locally:** 433/433 pgTAP assertions cover critical schema/grants, provider/product/location, notifications, direct messages and Chapter/Mission Spaces;
...
- **Verified on the clean local chain:** all eleven pgTAP files pass 433/433; `.github/workflows/database.yml` still needs retained remote-run evidence and a production-like snapshot upgrade;
