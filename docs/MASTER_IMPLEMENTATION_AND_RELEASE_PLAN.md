...
Status: authoritative execution plan, updated 28 August 2026
...
| Conversations                 | Direct-message paging/block/private Broadcast and a substantial Chapter/Mission Space system exist: paged Rooms/Threads/search, private Threads, reactions, mentions, attachments, attention/read state, Room permissions/slow mode, moderation and replay-safe in-tab sending | Add hosted browser personas, shared cross-surface store, DM/multi-device reconnect, presence/typing, Boards, scanner/worker activation and reliability/load tests                           |
| Action systems                | Missions, Chapters and related admin routes now prefer caller-bound creation/lifecycle/member/lead contracts                                                                                                                                                                   | Finish effective permission UI, moderation consistency, pagination/search, programme metrics and end-to-end proof                                                                           |
...
| I/O Port                      | Eight-view member shell, registry, v27 gateway, v9 compatible API, 5.5% accounting/finance controls, durable terminal metadata, packaged OpenCode client and five-provider/three-capacity inventory have Released/Verified slices; traffic evidence is zero                    | Commercial authorization, capped conformance/activation, direct token streaming, advanced billing units, scheduled operations, real-daemon evidence and reviewed inventory expansion remain |
| Supabase platform             | Connected Indus Orbit demo is healthy with 107 hosted migrations and current Space/I/O/Trust/Finance release contracts; latest hardens role-mention replay after Room slow mode                                                                                                | Historical aliases, current full empty/snapshot replay, generated-type remote CI, authenticated function audits and inherited performance-policy backlog remain                             |
...
### 3.1 Latest verified implementation delta — 28 August 2026
...
- The Chapter/Mission Space surface contains paged Rooms/Threads and permission-filtered search, private Threads, reactions/mentions, private attachments, read/attention state, Room permission administration and slow mode, reporting and bounded moderation. The member route now provides explicit offline state and replay-safe manual Room/Thread/attachment retry that keeps content only in open-tab memory. Shared cross-product state, DM/multi-device reconnect, Boards, presence/typing and retention remain programme work.
...
- the current 61 Security Advisor warnings require contract-by-contract review; new `conversation_*` objects have no attached warning;
- anonymous insert policies for contact and newsletter data require proper anti-abuse and data-minimisation controls;
- leaked-password protection configuration has not been independently verified through the available connector;
- current Performance Advisor results contain 166 warnings and 179 information notices; each inherited item needs ownership and evidence-based remediation;
...
7. verify leaked-password protection, enable it if disabled and configure production Auth protections;
...
- **Verified locally:** all 62 checked-in migrations replay from an empty database after three documented historical/environment recovery corrections;
- **Verified locally:** 446/446 pgTAP assertions cover critical schema/grants, provider/evidence/product/location, notifications, direct messages and Chapter/Mission Spaces;
- **Verified locally:** public/private schema lint and error-level Supabase security/performance advisors report no findings;
- **Released to demo:** all 62 current migrations are recorded; the Space addendum has no missing migration/table/function, 19/19 RLS tables, revoked protected direct writes and Realtime publication;
- **Security improvement:** anonymous execution of the identified privileged surface is closed and the legacy arbitrary-email function is a deployed 410 tombstone; the current 61 warning-level Advisor findings require classification/remediation;
- **Verified on the clean local chain:** all eleven pgTAP files pass 446/446; `.github/workflows/database.yml` still needs retained remote-run evidence and a production-like snapshot upgrade;
- **Trusted notification cutover Released:** protected domain actions own recipients/content, generic authenticated notification execution is revoked, the private email outbox is live and the former dispatcher is disabled; fixed-template worker deployment/operations remain;
...
- preserve the Released direct-message paging and Verified Space Room/Thread/attachment replay recovery; add one shared store, optimistic reconciliation, direct-message reconnect and multi-device unread resolution;
