...
Status: hosted project link and all 62 migrations are verified; the clean local suite passes 433/433 and the Chapter/Mission Space hosted contract passes. Historical timestamp aliases remain unreconciled, updated 9 August 2026.
...
The clean local database replays all 62 migrations and passes all 433 assertions across 11 pgTAP files. Generated TypeScript types reflect that schema. The new Chapter/Mission Space migration adds no uncovered foreign key. Hosted verification confirms no missing migration, Space table or expected function; all 19 public Space tables have RLS; protected direct Chapter/Mission writes are revoked; and Space messages belong to the Realtime publication.
...
Because 26 historical versions use different local/hosted timestamps, ordinary linked `db push` from the repository remains unsafe. This release pushed the compatible frontend first, fetched exact hosted history into a temporary view, added only the three forward migrations, proved the dry run listed only those files, and then applied them. No seed, role file, repair, reset or historical replay ran. Do not use migration repair to manufacture equality.
