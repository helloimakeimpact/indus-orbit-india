...
Hosted version `20260828175323` adds the bounded `slow_mode_seconds` Room policy, the `update_managed_conversation_room_v2(...)` manager boundary and a `(room_id, author_id, created_at desc)` covering index. `send_my_conversation_message(...)` now checks idempotent request reuse before slow mode, serializes each author's sends, returns exact bounded retry evidence and bypasses the delay only for current Space managers. The member Room administration dialog configures the twelve allowlisted intervals and shows the active policy in the Room header.

A rolled-back hosted member/manager fixture proved first send, same-ID idempotent retry, a second-send rejection with `retryAfterSeconds: 60` and two immediate manager sends. A cleanup read proved zero retained fixture messages and the Room policy restored to off. Anonymous execution of the manager command is denied. The local source is `20260828174751_release_orbit_room_slow_mode.sql`; the collaboration database contract now contains 43 assertions.
...
A rolled-back hosted fixture proved a two-item first page, one-item second page, no page overlap and continued exclusion of deleted content. Anonymous execution is denied and authenticated execution is explicit. The local source is `20260828082754_release_orbit_search_pagination.sql`; that phase raised the database contract to 38 assertions before the later slow-mode additions.

## 28 August 2026 private Thread audiences

Hosted versions `20260828081023`, `20260828081351` and `20260828081833` release atomic private Thread audience replacement and close both existing metadata-return paths. `get_my_conversation_thread_controls(uuid)` returns only caller-authorized visibility and explicit audience facts; `replace_managed_conversation_thread_members(uuid,uuid[])` is creator/Space-manager-only, serializes changes on the Thread row, caps the audience at thirty Room-eligible people and prevents creator removal. The caller-bound feed no longer projects private Thread title/count/lock metadata to non-members, and `create_my_conversation_thread(...)` no longer returns an existing private row to a caller outside its audience.

Rolled-back hosted fixtures proved creator controls, atomic two-person membership, non-manager replacement denial, creator retention, creator-visible/non-member-hidden feed summaries, non-member Thread-feed denial, non-member create/reuse denial and authorized reuse after audience admission. Anonymous execution of both new RPCs is denied. No fixture content or membership was retained. Local source files are `20260828080654_release_orbit_private_thread_controls.sql`, `20260828081253_harden_orbit_private_thread_feed.sql` and `20260828081747_harden_orbit_private_thread_reuse.sql`.

## 27 August 2026 Room permission editor

Hosted version `20260827231116` releases a manager-only list plus allow/deny/inherit editor for Room role/member overrides. It validates the exact Space subject and five-capability allowlist, deletes materialized state when policy returns to inherited, preserves source Chapter/Mission ownership, and rejects a manager directly denying their own Room visibility. Rolled-back hosted checks proved add/list/inherit removal, self-lockout rejection and non-manager denial. Both RPCs deny anonymous execution.

## 27 August 2026 bounded Orbit role mentions

Hosted version `20260827230122` releases manager-only role mentions and tightens person mentions to verify each recipient's actual Room/private-Thread visibility. A message may name at most three roles and expand to at most thirty distinct visible recipients; explicit people remain capped at ten. Role fan-out is rejected in private Threads, muted recipients are suppressed, and digest/quiet-hour scheduling uses the same private resolver as person mentions. Fixed notifications and outbox payloads contain no message content. Rolled-back hosted checks proved a manager mention, its durable role reference and non-manager rejection. Anonymous execution and member access to arbitrary-user visibility helpers are revoked.
...
Hosted version `20260827152257` releases validated per-Room/Space attention policy with an IANA timezone, optional quiet start/end, daily digest hour and deterministic next-delivery calculation. The same resolver drives caller-visible controls and mention outbox scheduling, so quiet/digest intent is not inferred in browser code. A rolled-back Asia/Kolkata contract at 23:30 local proved `quietActive=true` and a 07:00 local next-delivery timestamp; a caller-bound set/get contract proved the stored policy and next digest time. Hosted hardening version `20260827153347` makes arbitrary legacy JSON fall back to UTC/disabled/08:00 without a cast failure; the browser decoder also normalizes malformed display policy. The fixed-template delivery worker still needs deployment before queued external delivery can be called operational.
...
Hosted version `20260827144522` releases `list_my_io_api_key_usage(uuid)`. Active workspace members can see only current minute/day/month request counters, current day/month spent and reserved charge nanos, and reset times for their workspace keys. The raw key, hash, request content and private reservation records remain inaccessible. Anonymous execution is denied; caller-bound execution against a real active workspace returned a valid empty result because that workspace currently has no keys.

## 27 August 2026 Content and Programme admin operations

Hosted version `20260826144354` releases four authenticated-only RPCs for the separate admin app. Content queues expose bounded summaries for Story, Event, Course and S.O.D.A.; allowed transitions require exact current status, an 8–500-character reason and private audit evidence. Programme queues expose Chapter/Mission lifecycle evidence; the admin wrapper calls the canonical optimistic-concurrency transition functions so projected Orbit Space lifecycle follows its source. Anonymous execution is denied and all functions recheck the caller's exact scoped capability. A caller-bound super-admin read verified 64 Content rows and 9 Programme rows without mutating them.
...
The post-release hosted advisors report zero uncovered foreign keys and zero tables without primary keys. The remaining notices are 116 legacy auth-RLS initialization plans, 49 multiple-permissive-policy overlaps, 246 workload-dependent unused-index observations, 52 intentionally private RLS tables with no browser policy, 156 authenticated security-definer execution reviews and one project-level leaked-password-protection setting. The newest review covers the caller-bound Room-settings v2 boundary; authenticated execution is intentional and the function derives `auth.uid()`, validates authority and uses an empty search path. These notices are retained for explicit policy-by-policy review; no broad automated RLS rewrite is approved.
...
The hosted I/O inventory is not empty. The latest read-only inventory has 5 providers, 5 models, 5 endpoints, 5 endpoint capability versions, 6 price versions, 5 runtime controls, 5 endpoint connections, 3 capacity sources and 3 workspace capacity grants. It has 0 route receipts and 0 provider attempts. Inventory or stored secret-reference names are not proof of provider conformance or permission to create paid traffic.
...
- providers/models/endpoints/capabilities/runtime controls/connections: `5` each; price versions: `6`;
...
The latest connected Advisor run reports 209 security notices and 411 performance notices across the historical project. The new Room-settings v2 RPC notice is an intentional authenticated `SECURITY DEFINER` product boundary with internal caller/authority validation; the slow-mode lookup has a covering index and this phase introduced no RLS, foreign-key or primary-key defect. Advisor remediation remains tracked through the [Supabase database linter](https://supabase.com/docs/guides/database/database-advisors).
