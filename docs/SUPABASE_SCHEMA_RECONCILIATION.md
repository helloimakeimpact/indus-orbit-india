...
Status: hosted project link is authenticated; a clean local replay of all 62 checked-in migrations passes 433/433, while the hosted ledger still lacks the three newest forward migrations. Historical timestamp aliases remain unreconciled, updated 9 August 2026.
...
The following source migrations are not yet present in the hosted ledger and therefore remain **Verified locally**, not Released:
...
The clean local database replays all 62 migrations and passes all 433 assertions across 11 pgTAP files. Generated TypeScript types reflect that schema. The new Chapter/Mission Space migration adds no uncovered foreign key. This is strong local empty-replay evidence; hosted equivalence still requires exact forward deployment and hosted verification.
...
Because 26 historical versions use different local/hosted timestamps, ordinary linked `db push` from the repository remains unsafe. The forward release order is compatible frontend first, then the three exact migrations through the established temporary deployment view or project-scoped migration operation, followed by hosted schema/RLS/RPC/Realtime/persona checks. Do not use migration repair to manufacture equality.
...
This dated section records the earlier 58-migration evidence. It is superseded for local replay by the current clean 62-migration/433-assertion result. It still does **not** prove hosted equivalence: forward release, durable alias reconciliation, managed Realtime personas, a full schema-object comparison, generated-type drift automation and a production-like snapshot upgrade remain open gates.
...
## Additive changes: hosted releases and pending local forward migrations
...
| `20260809142000_create_trusted_product_event_rpcs.sql`        | **not hosted**                           | Adds atomic connection/mentor/mission/vouch/Chapter event RPCs and private email outbox.     | Verified within the clean local 62-migration/433-assertion suite; hosted release is pending.                                                 |
| `20260809150000_harden_email_delivery_claims.sql`             | **not hosted**                           | Hardens service-only email claim/lease behavior.                                             | Verified locally; hosted release is pending.                                                                                                 |
| `20260809152439_create_chapter_mission_space_foundation.sql`  | **not hosted**                           | Adds the caller-bound Chapter/Mission Space collaboration foundation and RLS/grants.         | Verified locally with focused schema/security/persona tests; hosted release is pending.                                                      |
...
2. Keep the proven fresh 62-migration/433-assertion replay required in CI and add a production-like snapshot upgrade before production.
...
4. Add a repeatable hosted drift gate; current declarations were regenerated from the clean local 62-migration schema and include the Space tables/RPCs.
