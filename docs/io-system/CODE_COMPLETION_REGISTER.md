...
| Supabase platform/data integrity      | Partial  | Hosted project `jpwvgpnbkrktipwhvqss` has 73 migrations, `io-gateway` v23, `io-openai` v4 and `io-provider-conformance` v1. The key/residency/conformance migrations and follow-up FK indexes are verified; generated TypeScript contracts are synchronized. The last clean local DB baseline remains 68/550.                                    | Repair/replace local containers for a complete 73-migration replay, keep generated-type drift automatic, retain alias-safe deployment, run hosted personas/contracts and establish staging/backup/restore.                                             |
