...
| Rooms/channels/categories       | Released blueprints/access plus manager name/description/posting/slow-mode controls and visible role/member allow/deny/inherit override editor                                                                              | Add create/reorder/archive, source-role assignment/hierarchy explanation and view-as-role tests.                                                                                           |
| Threads                         | Partial: Room-linked timelines, replies/counts, caller-bound follow/unfollow, per-Thread read pointers/unread state, moderated lock/reopen and atomic private membership are Released; member UI is locally Verified        | Add archive lifecycle and authenticated permission personas.                                                                                                                               |
...
| Mentions                        | Partial: explicit person plus manager-only Room/public-Thread role selection; ten people/three roles/thirty role recipients; actual Room/private-Thread authorization; quiet/digest-aware content-free fan-out are Released | Add worker delivery evidence, abuse analytics and authenticated multi-persona tests.                                                                                                       |
...
| Read/unread                     | Partial: DM behavior plus Space Room and followed-Thread read pointers/unread state are Released                                                                                                                            | Add cross-device attention reconciliation, preference-aware aggregate counts and accessibility announcements.                                                                              |
| Search                          | Released for each Space: indexed full-text search is caller-bound to accessible Rooms/private Threads, excludes deleted content, caps query/results and exposes stable keyset pages with Load more                          | Add consolidated saved-item search, optional ranking refinements and authenticated cross-space leakage/load personas.                                                                      |
...
| Moderation and safety           | Released slice: member reports, unified admin triage/assignment, reasoned content restrict/restore, Thread lock, database Room slow mode, scanner-evidence attachment review and independent appeals                        | Add block/mute/timeout/remove, automated spam controls and approved retention/export policy.                                                                                               |
...
7. **Trust and operations:** report triage, attachment review, independent appeals, permission overrides and Room slow mode are Released; complete timeout/spam controls, retention, export/delete and operational dashboards.
