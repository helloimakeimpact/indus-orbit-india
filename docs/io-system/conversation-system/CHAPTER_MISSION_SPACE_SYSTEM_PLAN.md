...
| Roles and Room overrides | Partial  | Manager allow/deny/inherit editor is Released; add source-role assignment/hierarchy where domain policy permits, effective-permission explanation and view-as-role tests.                                                      |
| Reactions and mentions   | Partial  | Reactions, explicit person mentions and manager-only Room role mentions are Released with bounded visibility and preference-aware scheduling; add public-Thread role-selection UI, worker delivery and abuse/persona evidence. |
...
| Search                   | Released | Caller-bound indexed full-text Space search rechecks Room/private-Thread access, excludes deleted content, caps input/results and opens Room/Thread context. Add cursor pagination and load personas before scale.             |
