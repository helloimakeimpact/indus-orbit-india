...
Status: canonical documentation hub, updated 9 August 2026 after the Chapter/Mission Space foundation was Verified locally.
...
| Conversations                         | Partial                      | Durable direct messages remain Released. A complete Chapter/Mission Space database foundation, deterministic Room blueprints, membership projection, caller-bound lifecycle/member/message/read RPCs and the first branded three-column Space UI are Verified locally. Hosted release, Threads UI, attachments, moderation, search and the cross-product shell remain.        |
...
| Data and Supabase                     | Partial                      | Project `jpwvgpnbkrktipwhvqss` is linked and authenticated. A clean local replay of all 62 checked-in migrations passes 433/433 assertions. The hosted ledger still lacks the trusted-event, email-claim and Chapter/Mission Space migrations, so those contracts remain Verified rather than Released. Historical timestamp reconciliation and production separation remain. |
