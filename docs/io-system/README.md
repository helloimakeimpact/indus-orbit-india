...
| Conversations                         | Partial                      | Durable direct messages, shared hooks and Realtime reconciliation exist. Message and domain-event RPCs plus the private notification/email outbox are Released; fixed-template worker deployment, common store, paging, private Broadcast, group collaboration and the shared spatial shell remain.     |
| I/O Port                              | Partial, deployed foundation | Control-plane schema, dynamic selection, gateway v17, receipts, fail-closed activation and UIs exist. Current provider/model/endpoint/capacity/control inventory is empty; operator provisioning, conformance and spend approval are required before routing.                                           |
...
| Data and Supabase                     | Partial                      | Ten current migrations are Released; 20/20 hosted checks and the upgraded local 376/376 database suite pass. Provider inventory/routing/traffic are zero. Historical timestamp reconciliation, fresh empty replay, Realtime and production separation remain.                                           |
| Quality and release operations        | Partial                      | Format, lint with zero errors, typecheck, 37 unit tests, 376 database assertions, production build and dependency audit pass. Fresh 61-migration replay, browser personas, conformance, telemetry, incident response and performance evidence remain.                                                   |
