...
Status: canonical documentation hub, updated 20 August 2026 after the transparent I/O fee, commercial provider gate and browser-key boundary release.
...
| I/O Port                              | Partial | Registry/dynamic selection/gateway v22/receipts, budgets/ledger, health/circuits, exact 5.5% fee evidence, commercial activation gate and scoped-key OpenAI-compatible v1 are Released. Persistent keys are blocked in browser-origin requests. OpenAI/DeepSeek remain resale-pending; traffic is zero.   |
...
| Quality and release operations        | Partial | Member unit tests pass 52/52; member format/type/build and admin 12-test/format/type/build pass. The prior 550-database-assertion and zero-production-vulnerability baselines remain. Hosted personas, accessibility, provider conformance, telemetry, incident response and performance evidence remain. |
