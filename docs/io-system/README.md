...
| Administration and operations         | Partial | `admin-indus-orbit` owns the standalone admin UI. Root admins are distinct from scoped duties; provider/team/evidence/budget/circuit operations, report triage, attachment review, appeals, finance evidence and the single-use USD 0.01-capped conformance backend are Released. The local repository passes 22/22 tests and enforced bundle budgets. Hosting, authenticated personas, MFA/two-person root and remaining content/program commands remain. |
...
| Quality and release operations        | Partial | Member tests pass 68/68 and admin tests pass 22/22; formatting, typecheck, production builds and dependency audits pass in both repositories. Both builds are vendor-split with enforced chunk budgets. Authenticated personas, accessibility/visual/real-daemon/load evidence, field Core Web Vitals, telemetry and incident-response drills remain.                                                                                                      |
