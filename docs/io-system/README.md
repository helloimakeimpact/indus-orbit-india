...
Status: canonical documentation hub, updated 20 August 2026 after the hosted bounded API-key, China-route and provider-conformance release.
...
| Administration and operations         | Partial | `admin-indus-orbit` owns the standalone admin UI and is published to its private GitHub `main`. Root admins are distinct from six scoped duties; provider/team/evidence/budget/circuit operations and the single-use USD 0.01-capped conformance backend are Released. Hosting, authenticated personas, MFA/two-person root and transactional trust/member/content/program commands remain.         |
...
| I/O Port                              | Partial | Registry/dynamic selection/`io-gateway` v23/receipts, budgets/ledger, health/circuits, exact 5.5% fee evidence, commercial activation gate, scoped-key `io-openai` v4, multi-window request limits, per-key spend caps, HMAC OpenAI safety IDs, explicit China-route policy and `io-provider-conformance` v1 are Released. OpenAI/DeepSeek remain resale-pending; traffic is zero.                  |
...
| Data and Supabase                     | Partial | Project `jpwvgpnbkrktipwhvqss` has 73 hosted migrations, gateway v23, `io-openai` v4 and `io-provider-conformance` v1. New objects, ACLs, defaults, route-off state and FK indexes verify; generated TypeScript types match hosted schema. The last clean local baseline remains 68/550, so no complete 73-migration local replay is claimed.                                                       |
