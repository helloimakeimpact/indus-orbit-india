...
Status: canonical documentation hub, updated 10 August 2026 after the I/O operational and terminal-metadata slices were Verified locally.
...
| Administration and operations         | Partial | `admin-indus-orbit` owns the standalone admin UI. Root admins are distinct from six scoped duties; provider/team/evidence operations are Released and budget/circuit operations are Verified locally. Hosting, MFA/two-person root and transactional trust/member/content/program commands remain.                                            |
...
| I/O Port                              | Partial | Registry/dynamic selection/gateway v18/receipts are Released to demo. Idempotency, hard budgets, balanced reserve/settle, health/circuits and matching member/admin UI are Verified locally. Hosted release, conformance and controlled activation remain.                                                                                    |
| I/O Terminal                          | Partial | Safe loopback OpenCode plus creator-only durable lifecycle metadata and hashed runtime references are Verified locally. Event streaming, resume, executable approvals, tools/diffs/artifacts/handoffs, daemon pairing and hosted runners remain.                                                                                              |
| Data and Supabase                     | Partial | Project `jpwvgpnbkrktipwhvqss` is linked. A clean 66-migration local replay passes 516 assertions; the prior 64-migration hosted Space/I/O boundary is filed as Released. Hosted migrations 65–66 are Blocked by missing CLI authentication.                                                                                                  |
| Quality and release operations        | Partial | Member typecheck, 43 unit tests, zero lint errors, build/format; admin 11 tests/typecheck/build/format; and 516 database assertions pass. Hosted personas, accessibility, provider conformance, telemetry, incident response and performance evidence remain.                                                                                 |
