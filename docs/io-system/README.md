...
Status: canonical documentation hub, updated 20 August 2026 after the transparent I/O fee release and local verification of bounded API-key, China-route and provider-conformance hardening.
...
| Administration and operations         | Partial | `admin-indus-orbit` owns the standalone admin UI. Root admins are distinct from six scoped duties; provider/team/evidence/budget/circuit database operations are Released. A reasoned, confirmed, USD 0.01-capped provider-conformance UI is Verified locally. Hosting, hosted conformance release, MFA/two-person root and transactional trust/member/content/program commands remain.                                                             |
...
| I/O Port                              | Partial | Registry/dynamic selection/gateway v22/receipts, budgets/ledger, health/circuits, exact 5.5% fee evidence, commercial activation gate and scoped-key OpenAI-compatible v1 are Released. Multi-window per-key limits, atomic per-key spend caps, HMAC OpenAI safety IDs, explicit China-route policy and the discovery-first conformance runner are Verified locally pending hosted release. OpenAI/DeepSeek remain resale-pending; traffic is zero. |
...
| Data and Supabase                     | Partial | Project `jpwvgpnbkrktipwhvqss` has all 71 source migrations, gateway v22 and `io-openai` v3. Migrations 68–71 and their hosted contracts pass. The last clean local baseline remains 68/550; the local database container is an infrastructure blocker, so no 71-migration replay is claimed.                                                                                                                                                       |
| Quality and release operations        | Partial | Member unit tests pass 54/54 and admin contract tests pass 13/13; final full format/type/build/audit rerun is recorded in the completion register. The prior 550-database-assertion baseline remains; the new database assertions await a working migration runner. Hosted personas, accessibility, controlled conformance execution, telemetry, incident response and performance evidence remain.                                                 |
