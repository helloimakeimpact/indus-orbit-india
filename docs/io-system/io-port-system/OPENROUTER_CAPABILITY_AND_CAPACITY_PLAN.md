...
| Provider ordering and allow/deny            | Partial: workspace owners/admins can explicitly admit or exclude China-hosted routes; the router fails closed and records the acknowledgement                                                                                                           | Extend the versioned workspace policy to other providers, capacity classes, regions, data-handling claims and explicit order.                                                        |
...
| Latency/throughput/availability routing     | Partial: endpoint health/circuit records, manual circuit controls and fail-closed routing exist                                                                                                                                                         | Add scheduled probes, rolling performance percentiles with sample confidence, automatic recovery and workspace policy.                                                               |
...
| Data collection/ZDR/residency policy        | Partial: evidence fields and explicit China-hosted/possible-training acknowledgement exist; unverified claims remain ineligible                                                                                                                         | Collect contractual evidence for every route; distinguish storage, transit, serving and training, and add further workspace controls.                                                |
...
| Workspaces, virtual keys and limits         | Partial: projects/environments, one-time hash-only keys, scopes, 30-day default/90-day maximum expiry, revocation, minute/day/month request limits, day/month spend caps, seven-day rotation warning and current caller-bound request/spend usage exist | Add operator suspension/anomaly policy, historical per-key analytics and production key classes after explicit plan review.                                                          |
...
| Credits, wallet and invoices                | Partial: integer-unit hard budgets, atomic reservations/settlement and distinct provider/5.5%-fee/customer totals exist                                                                                                                                 | Build wallet funding, double-entry credit ledger, adjustments, sponsorship expiry, GST/tax review, invoices and provider reconciliation.                                             |
