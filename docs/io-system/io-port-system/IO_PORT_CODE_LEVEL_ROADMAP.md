...
| Provider gateway         | `supabase/functions/io-gateway/index.ts`                              | Demo v28 is Released with bounded JSON transport, strict validation, transparent fee settlement, client-to-provider cancellation and corrected health evidence; scoped-key `io-openai` v12 adds direct upstream Chat/Responses SSE with terminal settlement. |
...
Chat JSON/direct SSE, the stateless Responses JSON/direct SSE subset, function tools, strict structured output and HTTPS image input are implemented behind exact capability gates. Direct streams decode split upstream events, forward text/tool deltas, settle only on the terminal provider event and release on cancellation; fallback stops once output begins. Still required before broad provider rollout: formal route-policy activation/retirement commands, activation of the existing health-probe schedule, distributed abuse/rate controls and provider-specific conformance for every enabled advanced capability. Unsupported modalities continue to fail closed.
