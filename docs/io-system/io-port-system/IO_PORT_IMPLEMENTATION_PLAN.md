...
Status: implementation baseline, updated 10 August 2026. The demo control plane, registry-driven `io-gateway` v18 and route-receipt schema are Released; five provider/model/endpoint records and three capacity sources/grants exist, while traffic remains zero pending secret-reference validation, conformance and controlled activation. Idempotency, hard budgets, route ledger, health/circuits and terminal metadata are Verified locally but not Released. This document remains the production-grade plan, not a claim that every planned surface is live. Verified operational status is maintained in `IO_PORT_IMPLEMENTATION_STATUS.md`.
