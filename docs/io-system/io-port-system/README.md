...
| Gateway                    | `io-gateway` v24 Released with no-dispatch route preflight plus the shared idempotent reserve/settle/circuit/receipt core, workspace region policy, per-key spend enforcement and versioned 5.5% fee settlement; JWT-protected, traffic disabled. |
...
| OpenAI-compatible API      | `io-openai` v4 Released: expiring scoped keys, immutable minute/day/month request and daily/monthly spend limits, browser-origin rejection, `/v1/models` and non-streaming `/v1/chat/completions`.                                                |
| Provider conformance       | `io-provider-conformance` v1 Released: single-use 30-minute approval, USD 0.01 ceiling, DeepSeek China acknowledgement, discovery first, one eight-token chat and redacted evidence. No run has been approved.                                    |
