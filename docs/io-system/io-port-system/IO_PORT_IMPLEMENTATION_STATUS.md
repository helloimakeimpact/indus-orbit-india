...
The activation-grade control-plane slice is **Released to the hosted demo**. Migration `20260810002754_create_io_operational_core.sql` and gateway v28 provide fingerprinted idempotency, hard budget reservation before dispatch, balanced settlement/release, health/circuits, bounded validated provider responses and client-to-provider cancellation. Gateway v28 and `io-openai` v12 share the same route transaction rules, with repaired endpoint-outcome recording; v12 adds hardened stream-terminal settlement. The terminal migrations add creator-only safe metadata and non-executable approval RPCs. Hosted release contracts pass; provider routing is still disabled.

The bounded OpenAI-compatible API foundation is also Released. Migration `20260819232624_add_io_openai_api_foundation.sql`, the member key-management UI and `io-openai` v12 provide one-time 30-day keys, SHA-256-only storage, scopes/revocation, membership revalidation, immutable multi-window request and spend limits, browser-origin persistent-key rejection, entitlement-filtered `/v1/models`, Chat JSON/direct SSE, a stateless `/v1/responses` JSON/direct SSE subset and provider-fetch cancellation. Function tools, strict JSON output and HTTPS image input are capability gated. See `OPENAI_COMPATIBLE_API_STATUS.md`. This is a usable compatibility subset, not the complete OpenRouter capability set.
...
| `npm run test:unit`                    | Pass — 88/88    | Auth/product/location, conversations/Orbit feed/attention/person-role-mention/permission/quiet/slow-mode/paged-search/private-Thread controls, API-key usage/rotation, terminal permission/credential policy, packaged OpenCode capability/SSE/tasks/diffs/tools/fork/abort/revert/permissions, gateway protocols/adapters/routing and fixed email templates are covered. |
...
| Hosted migrations/functions            | Released        | `io-gateway` v28, `io-openai` v12, `io-provider-conformance` v3 and `io-health-probe` v1 enforce their custom authentication and browser-origin/service-only boundaries.                                                                                                                                                                                                  |
...
| Provider adapter          | Released shared adapter has bounded OpenAI-compatible tools/results, strict structured output, HTTPS vision input and direct upstream text/tool SSE with split-frame decoding, terminal settlement and cancellation release; Gemini-native text remains non-streaming. The resolver consumes exact verified capability flags.                                                                    | No audio/files, Gemini-native streaming or passing advanced provider conformance evidence                                                                                     | Conform and activate each capability separately; add modalities and billing dimensions only with exact provider evidence                          |
...
| Reliability               | Released code requires idempotency, reserves retry cost, samples outcomes and excludes open circuits. Migration 111 plus `io-health-probe` v1 add service-authenticated non-billable discovery probes, three-failure automatic opening, successful-probe recovery, operator-hold preservation and redacted incident evidence. The route-outcome RPC argument is corrected and regression-tested. | The worker has no service-only schedule while eligible targets remain zero; rolling latency/queue scoring, external alert delivery and a distributed rate-limit layer remain. | Configure a five-minute schedule only after the first route is contract-approved; then load-test and rehearse open/recovery/kill-switch behavior. |
...
| Web control room          | Workspace, terminal, route/model selection and preflight, real budget state, paged/filterable usage, exact fee/credit/due evidence, credit/invoice summaries/downloads, durable terminal history, capacity and safe audit                                                                                                                                                                        | The member app uses the GitHub-to-Netlify delivery path; detailed health and authenticated live payment/reconciliation journeys are absent                                    | Confirm each production rollout, then add approved health and commercial operations                                                               |
...
| Conversation client       | One root Orbit store serves Community and I/O, with hosted private Broadcast policy, caller-bound keyset history, direct-message reconciliation, privacy-aware typing and replay-safe offline recovery                                                                                                                                                                                           | Authenticated multi-device and reconnect/permission persona evidence remains; structured Spaces is deliberately not persisted                                                 | Approve the structured-Spaces migration, then run two-device and source-role permission journeys                                                  |
...
- broader Responses/media compatibility, production live-key plans and authenticated/live-provider compatibility-load evidence; conservative beta limits, direct Chat/Responses SSE, terminal accounting, the stateless Responses subset and provider-request cancellation are Released.
