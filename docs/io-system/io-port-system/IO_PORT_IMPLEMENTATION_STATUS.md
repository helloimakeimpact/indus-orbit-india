...
| `npm run test:unit`                    | Pass — 80/80    | Auth/product/location, conversations/Orbit feed/attention controls, API-key usage/rotation, terminal permission/credential policy, packaged OpenCode capability/SSE/tasks/diffs/tools/fork/abort/revert/permissions, gateway protocols/adapters/routing and fixed email templates are covered. |
