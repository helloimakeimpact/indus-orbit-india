...
Hosted migrations `20260820191501`, `20260820191544` and `20260820191815` release the workspace China-processing policy, conservative key limits, atomic per-key spend reservations, single-use 30-minute/USD 0.01 provider-conformance approval, redacted evidence and covering audit indexes. `io-provider-conformance` v1 is active, but no approval or provider call has been created. The public admin source is separately published to the private `admin-indus-orbit` repository.
...
| `npm run test:unit`                    | Pass — 67/67    | Auth/product/location, conversations/Orbit feed, packaged OpenCode SSE/tasks/diffs/permissions, gateway protocols/adapters/routing and fixed email templates are covered. |
...
| Hosted migrations through 73/functions | Released        | `io-gateway` v23 preserves JWT, `io-openai` v4 enforces custom-key/browser-origin boundaries and `io-provider-conformance` v1 preserves JWT.                              |
...
| Provider adapter          | Released shared adapter has bounded OpenAI-compatible tool calls/results, strict structured output and HTTPS vision input plus Gemini-native text; the resolver consumes exact verified capability flags           | SSE is post-settlement rather than direct upstream; no disconnect cancellation, audio/files or passing advanced conformance evidence                                   | Conform and activate each capability separately, then add direct streaming/cancellation                      |
...
| Cost and budget           | Released integer-unit reserve/settle/release, exact 5.5% evidence, automatic non-cash credit application, caller-bound usage history and immutable draft invoice snapshots                                         | Cached input, tools/media, provider-specific units, reviewed FX, tax, invoice issuance/payment/refund and reconciliation remain                                        | Concurrency-test controlled traffic, then approve and implement the remaining commercial versions            |
...
| Web control room          | Workspace, terminal, route/model selection and preflight, real budget state, paged/filterable usage, exact fee/credit/due evidence, credit/invoice summaries, durable terminal history, capacity and safe audit    | The current member build is not hosted; detailed health, issued tax artifacts, payment and reconciliation are absent                                                   | Deploy and verify the member build, then add approved health and commercial operations                       |
| Local OpenCode            | Typed packaged loopback client, memory-only strong password, `/global/event` SSE plus REST reconciliation, continued prompts, task tree/todos, complete local diffs and hosted-audit-before-exact permission reply | Needs pinned real-daemon browser personas, capability negotiation, step-up, abort acknowledgement, checkpoints/artifacts, short-lived pairing and signed OS installers | Complete the production pairing/installer and real-daemon verification boundary                              |
...
- direct upstream streaming/cancellation, broader Responses compatibility, SDK/CLI examples, production live-key plans and compatibility/load evidence; conservative beta limits, post-settlement Chat SSE and the stateless Responses subset are Released.
...
This assessment uses local source, the last clean 68-migration/550-assertion replay, current member/admin checks and the live 73-migration Supabase schema. Production approval is not claimed. The three newest additive I/O migrations were released transactionally through the connected migration API and verified by object/ACL/default/state checks plus post-DDL advisors. Timestamp aliases still make ordinary linked pushes unsafe.
