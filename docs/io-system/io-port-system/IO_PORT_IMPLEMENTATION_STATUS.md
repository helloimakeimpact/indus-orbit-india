...
| `npm run test:unit`                    | Pass — 86/86    | Auth/product/location, conversations/Orbit feed/attention/person-role-mention/permission/quiet-policy/search/private-Thread controls, API-key usage/rotation, terminal permission/credential policy, packaged OpenCode capability/SSE/tasks/diffs/tools/fork/abort/revert/permissions, gateway protocols/adapters/routing and fixed email templates are covered. |
