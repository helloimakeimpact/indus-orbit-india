...
| `npm run test:unit`                    | Pass — 75/75    | Auth/product/location, conversations/Orbit feed/attention controls, API-key usage/rotation, packaged OpenCode capability/SSE/tasks/diffs/tools/fork/abort/revert/permissions, gateway protocols/adapters/routing and fixed email templates are covered. |
