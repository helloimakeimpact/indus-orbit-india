...
Operational truth, updated 4 September 2026: the registry-driven multi-provider foundation, latest-evidence resolver, `io-gateway` v28, scoped-key `io-openai` v12, single-use `io-provider-conformance` v3, top-level `/io` boundary, multi-window key/spend limits, budget/idempotency/ledger/health/circuit/cancellation controls, transparent 5.5% fee, commercial provider gate, terminal metadata and safe-timeline/approval boundary are Released to the demo. `io-openai` v12 adds direct upstream Chat/Responses SSE with terminal settlement. `io-health-probe` v1 is active behind platform JWT plus exact service-key authentication; it performs non-billable model discovery, records synthetic health, opens after three consecutive failures, recovers automatic circuits and preserves operator holds. It is not scheduled and has zero eligible targets. The database also contains fail-closed billing profiles, versioned GST/tax and FX evidence, approved-rate FX invoice conversion, immutable invoice issuance, payment/refund state, environment-bound signed-event idempotency and provider-statement reconciliation. `io-payments` v3 and `io-payment-webhook` v2 are active, but approved tax-policy, FX-rate and live-processor counts are zero; order creation therefore remains fail-closed. OpenAI and DeepSeek remain resale-pending. Secrets and inventory alone do not prove conformance or activate routing.
...
4. Current active functions are `io-gateway` v28, `io-openai` v12, `io-provider-conformance` v3 and `io-health-probe` v1. Each uses its reviewed authentication boundary; `io-openai` accepts scoped I/O keys and rejects browser-origin persistent keys, while the health worker requires an exact service key in addition to platform JWT verification.
