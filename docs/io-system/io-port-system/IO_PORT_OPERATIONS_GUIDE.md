...
Operational truth, updated 20 August 2026: the registry-driven multi-provider foundation, latest-evidence resolver, `io-gateway` v21, scoped-key `io-openai` v2, top-level `/io` boundary, budget/idempotency/ledger/health/circuit controls, terminal metadata and safe-timeline/approval boundary are Released to the demo. Gateway v21 bounds and validates provider responses and shares the route transaction with the API. Hosted contracts confirm RLS/grants/containment; the last inventory has five staged provider/model/endpoint/capability/price/runtime-control/connection records, three capacity sources/grants and zero receipts/attempts. Secrets and inventory alone do not prove conformance or activate routing. Read `IO_PORT_IMPLEMENTATION_STATUS.md` and `OPENAI_COMPATIBLE_API_STATUS.md` before any inventory or traffic change.
