...
4. Current active functions are `io-gateway` v28, `io-openai` v10, `io-provider-conformance` v3 and `io-health-probe` v1. Each uses its reviewed authentication boundary; `io-openai` accepts scoped I/O keys and rejects browser-origin persistent keys, while the health worker requires an exact service key in addition to platform JWT verification.
