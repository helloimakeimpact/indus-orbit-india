...
4. Add test secrets and configure the exact signed `io-payment-webhook` URL in Razorpay. The functions are deployed already but cannot create an order until an approved live processor record exists.
...
4. Current active functions are `io-gateway` v26, `io-openai` v8 and `io-provider-conformance` v3. Each uses its reviewed custom authentication boundary; `io-openai` specifically accepts scoped I/O keys and rejects browser-origin persistent keys.
