...
| Identity, profile and directory | `/auth`, top-level `/io`, opt-in `/onboarding`, `/app`, profile/directory/settings and caller-bound access/location/privacy RPCs | Partial; the I/O/Community boundary, country-first location and account export/deletion request intake are Released to demo. Production auth hardening, service-only export/purge execution and persona E2E remain. |
...
| Operations and governance       | standalone `admin-indus-orbit`, this repository's migrations, and compatibility admin routes                                     | Separate app has scoped Trust, Support, Content, Programme, Finance, Audit and I/O boundaries. Account-privacy review is Released; export/purge workers remain unavailable until policy approval.                   |
