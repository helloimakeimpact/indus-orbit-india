...
| Identity, profile and directory | `/auth`, top-level `/io`, opt-in `/onboarding`, `/app`, profile/directory/settings and caller-bound access/location RPCs | Partial; the I/O/Community boundary, country-first location and Settings withdrawal are Verified locally, while production auth hardening, privacy-safe discovery and persona E2E remain. |
