...
| 9router adoption      | Phase 0 Verified; Phase 1 Partial locally                      | Pinned source review, selection/rejection matrix, MIT notice and the new dependency-free `io-transport-core` package with twelve focused contracts. The loss-aware Anthropic request, JSON response and response-stream state machine are implemented but deliberately unregistered until loss-policy review/conformance is complete. No production traffic changed.     |
...
typecheck, 119/119 unit contracts, both package builds, production build/bundle
