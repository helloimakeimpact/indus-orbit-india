...
| 9router adoption      | Phases 0–1 Verified; Phase 2 Partial locally                   | Pinned source review, selection/rejection matrix, MIT notice and the dependency-free `io-transport-core` package with 28 focused contracts. Its bounded, loss-aware Anthropic and OpenAI Chat/Responses transforms are deliberately unregistered until conformance is complete. The new private execution-adapter package verifies signed route grants, exact-host HTTPS policy, injected secrets, cancellation, size/time/concurrency bounds and redacted failures in 11 contracts. It buffers responses and is not connected to production traffic. |
...
all three package builds, 148/148 unit contracts, production build/bundle
...
2. **Partial locally:** the private execution-adapter package and signed
   short-lived route grant are implemented. Add the streaming HTTP/service
   wrapper, service-auth/replay store and deployment-level DNS/egress controls.
...
Ordinary code work now: the streaming execution service wrapper, service-auth
and replay enforcement, feature-flag/shadow integration and local packaging can
proceed without exposing or spending a provider secret. The selected pure
9router-derived translator phase and the stateless adapter kernel are complete
at their local package boundaries.
