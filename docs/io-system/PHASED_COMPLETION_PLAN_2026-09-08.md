...
| 9router adoption      | Phase 0 Verified; Phase 1 Partial locally                      | Pinned source review, selection/rejection matrix, MIT notice and the dependency-free `io-transport-core` package with eighteen focused contracts. Loss-aware Anthropic and OpenAI Chat/Responses request, JSON response and response-stream transforms, fatal UTF-8 decoding and strict loss enforcement are implemented but deliberately unregistered until conformance is complete. No production traffic changed.                                                      |
...
typecheck, 125/125 unit contracts, both package builds, production build/bundle
...
1. Finish the Phase 1 adversarial nested-JSON fixtures and descriptor review
   from `io-port-system/NINEROUTER_SELECTIVE_ADOPTION.md`; the selected
   transforms, strict-loss policy and exhaustive UTF-8 split coverage are
   implemented.
