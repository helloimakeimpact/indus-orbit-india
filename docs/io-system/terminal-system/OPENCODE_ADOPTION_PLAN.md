...
| Tools and custom tools                   | Bounded local tool/command part identity, state and output trail is rendered; commands use the daemon catalogue and exact five-minute one-time review | Add MCP/tool schema/version allowlists, duration evidence and output classification rules.                                                                                         |
...
| Commands and prompt templates            | Verified daemon-advertised catalogue, visible-agent binding, bounded arguments and exact one-time confirmation; configured command agents are pinned  | Add compatible explicit model override, file references, ownership/version evidence and pinned real-daemon tests.                                                                  |
...
| Share links                              | Public OpenCode share is not adopted; explicit private local JSON handoff download is Verified                                                        | Add audience-bound, expiring, revocable I/O cloud handoffs with authorization rechecked on every open.                                                                             |
...
5. **Verified locally:** daemon-advertised reviewed command invocation and explicit private local handoff. Add MCP, LSP/formatter, repository context and typed cloud artifacts one bounded capability at a time.
