    ...
    <div className="min-h-screen bg-background">
      <div className="mx-auto max-w-4xl px-6 py-10">
        ...
        <header className="mt-8">
          ...
          {skill.summary && (
            <p className="mt-4 max-w-3xl text-lg text-foreground/75">{skill.summary}</p>
          )}
        </header>
            ...
            <ul className="mt-2 space-y-2">
              {skill.prerequisites.map((p, i) => (
                <li key={i} className="flex gap-2">
                  <span className="mt-1.5 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-[var(--saffron)]" />
                  {p.label}
                </li>
              ))}
            </ul>
        ...
        {skill.common_pitfalls && (
          <div className="mt-8 rounded-2xl border border-orange-200 bg-orange-50 p-5">
            <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-orange-900">
        ...
        {skill.india_context_notes && (
          <div className="mt-4 rounded-2xl border border-border bg-[var(--indigo-night)]/[0.04] p-5">
            <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-foreground/70">
              <MapPin className="h-3 w-3 text-[var(--saffron)]" /> India-context notes
            </p>
        ...
        {skill.templates?.length > 0 && (
          <Section title="Templates & resources">
            <ul className="mt-2 space-y-2">
              {skill.templates.map((t, i) => (
                <li key={i}>
                  <a
                    href={t.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 text-sm font-medium text-[var(--indigo-night)] hover:underline"
                  >
