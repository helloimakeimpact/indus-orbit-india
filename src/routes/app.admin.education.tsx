...
export const Route = createFileRoute("/app/admin/education")({
  head: () => ({
    meta: [{ title: "Education admin — Indus Orbit" }, { name: "robots", content: "noindex" }],
  }),
  component: EducationAdmin,
});

type AdminCourse = Awaited<ReturnType<typeof listAllCoursesForAdmin>>[number];
...
type LessonAttachment = Database["public"]["Tables"]["lesson_attachments"]["Row"];
type QuizEditorData = Awaited<ReturnType<typeof getQuizForEditing>>;
type QuizQuestion = QuizEditorData["questions"][number];
type QuizOption = QuizQuestion["options"][number];
type AdminResource = Awaited<ReturnType<typeof listAllResourcesForAdmin>>[number];
type ResourceForm = {
  kind: "link" | "file";
  title: string;
  description: string;
      ...
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage courses, lessons, quizzes, and the resource library.
          </p>
        </div>
      </header>
...
function CoursesList({ onOpen }: { onOpen: (slug: string) => void }) {
  ...
  async function load() {
    try {
      setItems(await listAllCoursesForAdmin());
    } catch (error) {
  ...
  async function setStatus(course: AdminCourse, status: string) {
    try {
  ...
  async function onDelete(course: AdminCourse) {
    if (!confirm(`Delete "${course.title}" and all its modules/lessons?`)) return;
    try {
          ...
          {items.map((c) => (
            <Card key={c.id}>
              <CardContent className="flex items-center justify-between gap-4 p-4">
                ...
                <div className="flex items-center gap-2">
                  <Button size="sm" variant="outline" onClick={() => onOpen(c.slug)}>
                    <Pencil className="h-4 w-4" /> Edit
                  </Button>
                  {c.status !== "published" ? (
                    <Button size="sm" onClick={() => setStatus(c, "published")}>
                      Publish
                    </Button>
                  ) : (
                    <Button size="sm" variant="outline" onClick={() => setStatus(c, "draft")}>
                      Unpublish
                    </Button>
                  )}
...
function ModulesEditor({
  ...
  async function renameModule(module: CourseModule, title: string) {
    try {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-lg">Modules & lessons</CardTitle>
      </CardHeader>
...
function ModuleRow({
  ...
  onRename: (module: CourseModule, title: string) => void;
  onChanged: () => void;
  onEditLesson: (lesson: CourseLesson) => void;
}) {
  ...
  async function setLessonStatus(lesson: CourseLesson, status: string) {
    try {
    ...
    <div className="rounded-md border p-3 space-y-2">
      <div className="flex items-center gap-2">
        <Input
          ...
          onBlur={() => title !== m.title && onRename(m, title)}
        ...
        {m.lessons.map((l) => (
          ...
          >
            ...
            <div className="flex items-center gap-1">
              <Button size="sm" variant="outline" onClick={() => onEditLesson(l)}>
                <Pencil className="h-3 w-3" />
              </Button>
              {l.status === "draft" ? (
                <Button size="sm" onClick={() => setLessonStatus(l, "published")}>
                  Publish
                </Button>
              ) : (
                <Button size="sm" variant="outline" onClick={() => setLessonStatus(l, "draft")}>
                  Unpublish
                </Button>
              )}
              <Button size="sm" variant="ghost" onClick={() => removeLesson(l.id)}>
...
function LessonEditor({
  ...
  async function onFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!f) return;
    try {
      await addLessonAttachment(lesson.id, f);
  ...
  async function removeAttachment(attachment: LessonAttachment) {
    try {
      await deleteLessonAttachment(attachment.id, attachment.file_path);
      reload();
    } catch (error) {
      toast.error(getErrorMessage(error));
    }
...
function QuizEditor({
  ...
  async function saveMeta() {
    try {
      await upsertQuiz(lessonId, title, passing);
  ...
  async function addQ() {
    ...
    try {
      await addQuizQuestion(qid, newQ);
      setNewQ("");
      onChanged();
    } catch (error) {
      toast.error(getErrorMessage(error));
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex gap-2">
        ...
        {data.quiz && (
          <Button size="sm" variant="ghost" onClick={removeQuiz}>
...
function QuestionEditor({
  ...
  async function addOpt() {
    if (!label) return;
    try {
      await addQuizOption(q.id, label, correct);
      setLabel("");
      setCorrect(false);
      onChanged();
    } catch (error) {
      ...
      <div className="space-y-1 pl-2">
        {q.options.map((o: QuizOption) => (
          <div key={o.id} className="flex items-center justify-between text-sm">
            ...
            <Button size="sm" variant="ghost" onClick={() => removeOpt(o.id)}>
              <Trash2 className="h-3 w-3" />
            </Button>
          </div>
        ))}
...
function ResourcesManager() {
  ...
  async function load() {
    try {
      setItems(await listAllResourcesForAdmin());
    } catch (error) {
      toast.error(getErrorMessage(error));
    }
  }
  ...
  async function toggleStatus(resource: AdminResource) {
  ...
  async function remove(resource: AdminResource) {
            ...
            <div className="space-y-3">
              <div className="flex gap-2">
                <Button
                  type="button"
                  size="sm"
                  variant={form.kind === "link" ? "default" : "outline"}
                  onClick={() => setForm({ ...form, kind: "link" })}
                >
                  Link
                </Button>
                <Button
                  type="button"
                  size="sm"
                  variant={form.kind === "file" ? "default" : "outline"}
