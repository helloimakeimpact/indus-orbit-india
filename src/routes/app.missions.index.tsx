...
export const Route = createFileRoute("/app/missions/")({
  head: () => ({
    meta: [{ title: "India Missions — Indus Orbit" }, { name: "robots", content: "noindex" }],
  }),
      ...
      <div className="app-glass app-workspace-head rounded-2xl">
        <div className="app-workspace-head-main">
          <div className="app-workspace-title">
            <p className="app-workspace-kicker">🇮🇳 India Missions</p>
            <h1>Platform Campaigns</h1>
            <p>
              Time-bounded campaigns where Diaspora members and Experts commit capital, mentorship,
              or hiring support toward a cohort of Founders.
            </p>
          </div>
          <div className="flex items-start justify-end gap-2">
            {isAdmin && (
              <Button
                ...
                className="h-8 rounded-full bg-[var(--indigo-night)] px-3 text-[var(--parchment)]"
              >
                <Plus className="mr-1 h-4 w-4" /> 🚀 Create Mission
              </Button>
            )}
        ...
        <div className="app-workspace-controls">
          <div className="app-filter-row">
            {STATUS_TABS.map((tab) => (
              <button
                ...
                className="app-chip"
                data-active={statusFilter === tab}
              >
                {tab === "all"
                  ? `🌐 All (${allMissions.length})`
                  : tab === "open"
                    ? `🟢 Open (${allMissions.filter((m) => m.status === tab).length})`
                    : tab === "completed"
                      ? `✅ Completed (${allMissions.filter((m) => m.status === tab).length})`
                      : `🗄️ Archived (${allMissions.filter((m) => m.status === tab).length})`}
              </button>
            ))}
          </div>
          ...
          {filtered.length > 0 && (
            <p className="app-result-line">
              Showing {visible.length} of {filtered.length} mission
              {filtered.length !== 1 ? "s" : ""}
            </p>
          )}
        </div>
        ...
        {filtered.length === 0 ? (
          ...
          visible.map((m) => {
            ...
            return (
              <div
                key={m.id}
                className="overflow-hidden rounded-2xl border border-border bg-card transition hover:border-foreground/20"
              >
                <div className="border-b border-border p-3.5">
                  <div>
                    <div className="flex items-start justify-between gap-3">
                      <Badge className="bg-[var(--saffron)] text-[var(--indigo-night)] text-[10px] uppercase tracking-widest">
                        {m.theme}
                      </Badge>
                      <Badge variant="outline">{m.status.toUpperCase()}</Badge>
                    </div>
                    <h2 className="mt-3 max-w-2xl text-lg font-semibold">{m.title}</h2>
                ...
                <div className="p-3.5 md:flex md:items-center md:justify-between">
                  <div className="flex flex-wrap gap-4 text-sm">
                    {Object.entries(segmentCounts).map(([segment, count]) => {
                      const meta =
                        segment !== "other"
                          ? SEGMENT_META[segment as keyof typeof SEGMENT_META]
                          : null;
                      ...
                      return (
                        <div key={segment} className="flex items-center gap-2">
                          <div
                            className={`flex h-7 w-7 items-center justify-center rounded-full ${colors.bg} ${colors.text}`}
                          >
                ...
                {isAdmin && (
                  <div className="border-t border-border p-4 px-6 bg-muted/30 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        size="sm"
                        variant={m.status === "archived" ? "default" : "outline"}
    ...
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        ...
        <DialogFooter>
          ...
          <Button
            onClick={submit}
            disabled={busy || !form.title || !form.theme || !form.description}
          >
            Create
          </Button>
        </DialogFooter>
...
function JoinMissionDialog({
  ...
  const [role, setRole] = useState<"contributor" | "founder">(
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="e.g. I can review pitch decks on weekends..."
            />
          </div>
        </div>
        <DialogFooter>
