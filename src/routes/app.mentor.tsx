...
export const Route = createFileRoute("/app/mentor")({
  head: () => ({
    meta: [{ title: "Mentorship — Indus Orbit" }, { name: "robots", content: "noindex" }],
  }),
    ...
    <div className="mx-auto w-full max-w-7xl">
      ...
      {data.asBooker.length > 0 && (
        <section className="mt-10">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            {data.asBooker.map((s) => (
              <SessionCard key={s.id} session={s} isExpert={false} onUpdate={updateStatus} />
            ))}
          </div>
        </section>
      )}
    ...
    <div className="rounded-2xl border border-border bg-card p-5">
      ...
      {session.status === "accepted" && session.meeting_url && (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="mt-2 inline-flex items-center gap-1 text-sm text-[var(--saffron)] hover:underline"
        >
      ...
      <div className="mt-5 flex gap-2">
        {session.status === "pending" && isExpert && (
          <>
            ...
            <Button size="sm" variant="outline" onClick={() => onUpdate(session.id, "declined")}>
              <X className="mr-1 h-3.5 w-3.5" /> Decline
            </Button>
          </>
        )}
        {session.status === "accepted" && isExpert && (
          <Button size="sm" variant="outline" onClick={() => onUpdate(session.id, "completed")}>
            <CheckCircle className="mr-1 h-3.5 w-3.5" /> Mark Completed
          </Button>
        )}
        {session.status === "pending" && !isExpert && (
          <Button size="sm" variant="outline" onClick={() => onUpdate(session.id, "cancelled")}>
            Cancel Request
          </Button>
        )}
      </div>
    ...
    <Dialog open={true} onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        ...
        <DialogFooter>
          ...
          <Button onClick={submit} disabled={busy || !scheduledFor}>
