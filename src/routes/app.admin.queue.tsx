...
export const Route = createFileRoute("/app/admin/queue")({
  head: () => ({
    meta: [{ title: "Admin Queues — Indus Orbit" }, { name: "robots", content: "noindex" }],
  }),
  ...
  return (
    <div className="mx-auto w-full max-w-7xl">
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        Manage verifications and chapter proposals.
      </p>
                ...
                {rows.map((p) => (
                  <article key={p.id} className="rounded-3xl border border-border bg-card p-6">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-1 text-xs uppercase tracking-wider text-muted-foreground">
                          {formatInlineProfileLocation(p)} · joined{" "}
                          {new Date(p.created_at).toLocaleDateString()}
                        </p>
                      </div>
                      <div className="flex flex-wrap items-center gap-2">
                        {p.orbit_segment && (
                          <Badge variant="secondary">{SEGMENT_META[p.orbit_segment].label}</Badge>
                        )}
                    ...
                    <div className="mt-3 flex flex-wrap gap-3 text-sm">
                      {p.linkedin_url && (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="text-[var(--indigo-night)] underline"
                        >
                          LinkedIn
                        </a>
                      )}
                    ...
                    {p.segment_details && Object.keys(p.segment_details).length > 0 && (
                      <details className="mt-3">
                        ...
                        <pre className="mt-2 overflow-x-auto rounded-xl bg-muted/40 p-3 text-xs">
                          {JSON.stringify(p.segment_details, null, 2)}
                        </pre>
                      </details>
                    )}
                    <Textarea
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      ...
                      <Button variant="outline" onClick={() => decide(p, "needs_more_info")}>
                        Needs more info
                      </Button>
                      <Button variant="outline" onClick={() => decide(p, "declined")}>
                        Decline
                      </Button>
                    </div>
                  </article>
          ...
          <TabsContent value="chapters" className="pt-6 outline-none">
            {proposals.length === 0 ? (
              <p className="mt-8 text-muted-foreground">
                Inbox zero — no chapter proposals pending.
              </p>
            ) : (
              <div className="space-y-4">
                {proposals.map((prop) => (
                  <article key={prop.id} className="rounded-3xl border border-border bg-card p-6">
                    ...
                    <div className="space-y-4 mt-4 bg-muted/20 p-4 rounded-xl text-sm">
                      <div>
                        ...
                        {prop.profiles?.display_name || "Unknown"}
                      </div>
                      <div>
