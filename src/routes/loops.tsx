...
export const Route = createFileRoute("/loops")({
  head: () => ({
    meta: [
      { title: "Loops — Indus Orbit" },
      {
        name: "description",
        content:
          "AI and agent iteration loops for building, evaluating and improving India-focused workflows.",
      },
      { property: "og:title", content: "Loops — Indus Orbit" },
      {
        property: "og:description",
        content:
          "Execution loops for AI systems: baseline, eval set, rerun trigger, stack and business fit.",
      },
    ],
  }),
  component: LoopsPage,
      ...
      <section className="bg-[var(--indigo-night)] px-6 pb-16 pt-32 text-[var(--parchment)]">
        <div className="mx-auto max-w-6xl">
          <span className="inline-flex items-center gap-2 rounded-full border border-[var(--parchment)]/20 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-[var(--saffron)]">
            <Workflow className="h-3.5 w-3.5" /> Agent operations
          </span>
        ...
        <div className="mx-auto max-w-6xl">
          <div className="flex flex-col gap-4 border-b border-border pb-5 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <h2 className="mt-2 font-display text-3xl font-medium">Published loops</h2>
            </div>
          ...
          {!loaded ? (
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              {filtered.map((loop) => (
                <article key={loop.id} className="rounded-2xl border border-border bg-card p-5">
                  ...
                  <h3 className="mt-4 font-display text-xl font-medium">{loop.title}</h3>
                  {loop.summary && (
            ...
            <div className="mt-6 rounded-2xl border border-dashed border-border bg-card p-8">
              ...
              <h3 className="mt-4 font-display text-2xl font-medium">Loops are being prepared.</h3>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                This library will connect S.O.D.A opportunities to concrete AI workflows: evals,
                agents, automation, benchmarks and iteration history.
              </p>
              <Link
                to="/skills"
