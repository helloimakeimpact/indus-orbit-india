...
export const Route = createFileRoute("/app/admin/content")({
  head: () => ({
    meta: [{ title: "Content moderation — Admin" }, { name: "robots", content: "noindex" }],
  }),
...
function ContentAdmin() {
  ...
  const audit = useCallback(
    async (action: string, target_type: string, target_id: string, reason?: string) => {
      if (!user) return;
      await supabase
        .from("audit_log")
        .insert({ actor_id: user.id, action, target_type, target_id, reason });
    },
    [user],
  );
  ...
  return (
    <div className="mx-auto w-full max-w-7xl">
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        Approve, pause, feature, or remove content across the platform. All actions are logged in
        the audit trail.
      </p>
        ...
        <Tabs defaultValue="stories" className="mt-8">
                ...
                >
                  ...
                  {s.status !== "featured" && (
                    <Btn
                      icon={Star}
                      variant="outline"
                      onClick={() => setStoryStatus(s.id, "featured")}
                    >
                      Feature
                    </Btn>
                  )}
                  {(s.status === "approved" || s.status === "featured") && (
                    <Btn
                      icon={Pause}
                      variant="outline"
                      onClick={() => setStoryStatus(s.id, "pending")}
                    >
                      Pause
                    </Btn>
                  )}
                  {s.status !== "rejected" && (
                    <Btn
                      icon={XCircle}
                      variant="outline"
                      onClick={() => setStoryStatus(s.id, "rejected")}
                    >
                      Decline
                    </Btn>
                  )}
                ...
                >
                  ...
                  {e.status === "approved" && (
                    <Btn
                      icon={Pause}
                      variant="outline"
                      onClick={() => setEventStatus(e.id, "pending")}
                    >
                      Pause
                    </Btn>
                  )}
                ...
                >
                  ...
                  {a.status === "active" && (
                    <Btn
                      icon={Pause}
                      variant="outline"
                      onClick={() => setAskStatus(a.id, "paused")}
                    >
                      Pause
                    </Btn>
                  )}
                  {a.status !== "closed" && (
                    <Btn
                      icon={XCircle}
                      variant="outline"
                      onClick={() => setAskStatus(a.id, "closed")}
                    >
                      Close
                    </Btn>
                  )}
          ...
          <TabsContent value="missions" className="mt-6 space-y-4">
                ...
                >
                  ...
                  {m.status !== "closed" && (
                    <Btn
                      icon={XCircle}
                      variant="outline"
                      onClick={() => setMissionStatus(m.id, "closed")}
                    >
                      Close
                    </Btn>
                  )}
            ...
            <p className="text-xs text-muted-foreground">
              Missions cannot be deleted — close them instead to preserve history.
            </p>
          </TabsContent>

          <TabsContent value="chapters" className="mt-6 space-y-4">
            {chapters.length === 0 ? (
              <Empty label="chapters" />
            ) : (
              chapters.map((c) => (
                <Card
                  key={c.id}
