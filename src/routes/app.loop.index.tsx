...
function LoopIndex() {
  ...
  const filtered = useMemo(() => {
    ...
    if (needle)
      list = list.filter((r) =>
        [r.title, r.summary, r.domain, ...r.tags].join(" ").toLowerCase().includes(needle),
      );
    return sort === "signal"
    ...
    <div className="min-h-screen bg-background">
      <div className="mx-auto max-w-7xl px-6 py-10">
        ...
        <p className="mt-2 max-w-2xl text-sm text-foreground/60">
          Working iteration loops — problem, eval set, minimum pipeline, and the trigger to re-run
          on the next frontier model.
        </p>
