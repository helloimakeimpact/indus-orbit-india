...
function LoopDetail() {
  ...
  if (!loop)
    return (
      <div className="p-12">
        <Link to="/app/loop" className="inline-flex items-center gap-2 text-sm text-foreground/70">
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="mx-auto max-w-4xl px-6 py-10">
        ...
        >
          <ArrowLeft className="h-4 w-4" /> Back to Loop
        </Link>
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-4">
          ...
          <Stat
            icon={IndianRupee}
            label="Cost / iteration"
            value={l.cost_per_iteration_inr ? `₹${l.cost_per_iteration_inr}` : "—"}
          />
          <Stat
            icon={Timer}
            label="Latency target"
            value={l.latency_target_ms ? `${l.latency_target_ms} ms` : "—"}
          />
        ...
        {l.stack?.length > 0 && (
          <div className="mt-8 flex flex-wrap gap-2">
            <p className="w-full text-xs font-semibold uppercase tracking-wider text-foreground/60">
              Stack
            </p>
            {l.stack.map((t) => (
        ...
        {l.related_soda_slug && (
          <div className="mt-8 rounded-2xl border border-border bg-[var(--indigo-night)]/[0.04] p-5">
            <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-foreground/70">
              <Repeat className="h-3 w-3 text-[var(--saffron)]" /> Powers a S.O.D.A idea
            </p>
            <Link
              to="/app/soda/$slug"
              params={{ slug: l.related_soda_slug }}
...
function Box({ label, value }: { label: string; value?: string }) {
  return (
    <div className="rounded-2xl border border-border bg-card p-4">
      <p className="text-[11px] font-semibold uppercase tracking-wider text-[var(--saffron)]">
        {label}
      </p>
