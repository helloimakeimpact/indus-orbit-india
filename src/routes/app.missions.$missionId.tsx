      ...
      <div className="rounded-3xl border border-border bg-card overflow-hidden">
        <div className="bg-[var(--indigo-night)] p-8 text-[var(--parchment)] relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="mt-4 text-[var(--parchment)]/80 max-w-3xl leading-relaxed">
              {mission.description}
            </p>
          </div>
        </div>
              ...
              {!hasJoined ? (
                <div className="rounded-2xl border border-dashed border-border p-8 text-center bg-muted/20">
                  <p className="text-muted-foreground">
                    You must join this mission to view or post updates.
                  </p>
                </div>
              ) : (
                <>
                              ...
                              <div>
                                <div className="flex items-center gap-2">
                                  <p className="text-sm font-semibold">
                                    {u.profiles?.display_name || "Member"}
                                  </p>
                                  {u.is_pinned && (
                                    <Badge
                                      variant="secondary"
                                      className="text-[10px] bg-[var(--saffron)]/10 text-[var(--saffron)] hover:bg-[var(--saffron)]/20 px-1.5 py-0"
                                    >
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <h4 className="font-medium text-lg">
                          <Link to={`/app/events/${evt.id}`} className="hover:underline">
                            {evt.title}
                          </Link>
                        </h4>
                        <p className="text-sm text-muted-foreground capitalize">
                          {new Date(evt.start_time).toLocaleDateString()} •{" "}
                          {formatLocationTypeLabel(evt.location_type)} • {formatEventLocation(evt)}
                        </p>
                      </div>
                    </div>
                  ))
                ...
                {mission.stories?.length === 0 ? (
                  ...
                  mission.stories?.map((story: any) => (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <h4 className="font-medium text-lg">
                          <Link to={`/app/stories/${story.id}`} className="hover:underline">
                            {story.title}
                          </Link>
                        </h4>
                        <p className="text-sm text-muted-foreground capitalize">
