...
import { ArrowRight, Sparkles, Search, Repeat, Zap, Flame } from "lucide-react";

export const Route = createFileRoute("/loop")({
  head: () => ({
    meta: [
      { title: "Loop — Build the loop, not the agent | Indus Orbit" },
      { name: "description", content: "A library of AI iteration loops built for India: problem, eval, minimum pipeline, and the trigger to re-run on the next frontier model." },
      { property: "og:title", content: "Loop — Indus Orbit" },
      { property: "og:description", content: "AI iteration-loop blueprints for India-scale builders." },
...
function score(l: LoopEntry) {
  return (l.score_iteration_speed + l.score_eval_rigor + l.score_business_value + l.score_india_fit) / 4;
}
...
function LoopPublic() {
  ...
  const publicList = useMemo(() => {
    ...
    const seen = new Set<string>(); const out: LoopEntry[] = [];
  ...
  const filtered = publicList.filter(l => !q.trim() || [l.title, l.summary, l.domain, ...l.tags].join(" ").toLowerCase().includes(q.toLowerCase()));
    ...
    <SiteShell navTone="dark">
      <section className="relative w-full overflow-hidden bg-[var(--indigo-night)] pt-36 pb-20 text-[var(--parchment)]">
        <div className="relative z-10 mx-auto max-w-5xl px-6 text-center">
          ...
          <h1 className="mt-6 font-display text-5xl font-light leading-[1.05] text-glow md:text-7xl">Loop</h1>
          <p className="mt-3 font-display text-xl text-[var(--saffron)] md:text-2xl">Build the loop, not the agent.</p>
          <p className="mx-auto mt-6 max-w-2xl text-base text-[var(--parchment)]/85 md:text-lg">
            Every entry is a working iteration loop — problem, eval set, minimum pipeline, and the trigger that re-runs it on the next frontier model.
          </p>
          <form onSubmit={(e) => e.preventDefault()} className="relative mx-auto mt-10 flex max-w-2xl items-center gap-2 rounded-full border border-[var(--parchment)]/25 bg-[var(--parchment)]/10 px-2 py-2 backdrop-blur-md">
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search loops, domains, models…" className="w-full bg-transparent px-2 py-2 text-sm text-[var(--parchment)] placeholder:text-[var(--parchment)]/60 focus:outline-none" />
      ...
      {ofDay && (
        <section className="px-6 py-20">
          <div className="mx-auto w-full max-w-7xl">
            <p className="text-xs font-semibold uppercase tracking-[0.25em] text-[var(--saffron)]">Loop of the day</p>
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-4">
              <MiniStat icon={Repeat} label="Baseline" value={ofDay.current_baseline_model ?? "—"} />
              <MiniStat icon={Zap} label="Re-run trigger" value={(ofDay.trigger_to_rerun ?? "—").slice(0, 60)} />
              <MiniStat icon={Flame} label="Cost/iter" value={ofDay.cost_per_iteration_inr ? `₹${ofDay.cost_per_iteration_inr}` : "—"} />
        ...
        <div className="mx-auto w-full max-w-7xl">
          <div className="mb-6 flex items-end justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 font-display text-3xl font-medium md:text-4xl">Loops the Orbit runs.</h2>
              <p className="mt-2 text-sm text-foreground/60">Top-signal and freshest loops. <Link to="/auth" className="font-semibold text-[var(--indigo-night)] underline">Sign in</Link> for all {rows.length} entries.</p>
            </div>
          </div>
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {filtered.map(l => <LoopCard key={l.id} l={l} />)}
          </div>
        </div>
      </section>
    </SiteShell>
